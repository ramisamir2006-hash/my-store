...
MY_CHANNEL = '@RamySamir2026Gold'  # القناة الخاصة بالمتجر
SUPPORT_USER = '@RamiSamir2024'    # حساب التواصل
...
print(f"✅ Linked Channel: {MY_CHANNEL}")
...
# ====================================================
# --- بيانات الربط النهائية لـ RAMY SAMIR ---
# ====================================================
BOT_TOKEN = '8395659007:AAHaIQBJD_dTd6Np46fNeNS-WHoAbLNK0rk'
ADMIN_ID = 7020070481              # هويتك كمدير
CHANNEL_ID = '@RamySamir2026Gold'  # القناة العامة
SUPPORT_USER = '@RamiSamir2024'    # حساب التواصل
STORE_CURRENCY = 'EGP'             # العملة
# ====================================================

# إعداد البوت (استخدام نظام Polling بدلاً من Webhook لسهولة التشغيل)
bot = telebot.TeleBot(BOT_TOKEN, threaded=False)
...
# --- وظيفة إنشاء لوحة المفاتيح الرئيسية ---
def create_main_keyboard():
    keyboard = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
    keyboard.row_width = 2
    keyboard.add(
        types.KeyboardButton(text="Shop Items 🛒"),
...
def send_welcome(message):
    ...
    try:
        # إحصائيات سريعة من قاعدة البيانات الخاصة بك
        all_users = GetDataFromDB.AllUsers()[0][0] if GetDataFromDB.AllUsers() else 0
        all_products = GetDataFromDB.AllProducts()[0][0] if GetDataFromDB.AllProducts() else 0
        
        if user_id == ADMIN_ID:
            keyboardadmin = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
            keyboardadmin.add("Manage Products 💼", "Manage Categories 💼")
            keyboardadmin.add("Manage Orders 🛍", "Payment Methods 💳")
            keyboardadmin.add("News To Users 📣", "Switch To User 🙍‍♂️")
            
            stats = f"📊 إحصائيات المتجر:\n👤 المستخدمين: {all_users}\n📦 المنتجات: {all_products}"
            bot.send_message(user_id, f"أهلاً رامي! 🤴\n{stats}\n\nتحكم بمتجرك الآن:", reply_markup=keyboardadmin)
        else:
            bot.send_message(user_id, "مرحباً بك في متجرنا! 🛍\nتصفح المنتجات واختر ما يناسبك.", reply_markup=create_main_keyboard())
    except Exception as e:
        logger.error(f"Start Error: {e}")
        bot.send_message(user_id, "مرحباً! اضغط على القائمة للبدء.")
...
def callback_query(call):
    try:
        if call.data.startswith("getcats_"):
            CategoriesDatas.get_category_products(call, call.data.replace('getcats_', ''))
        elif call.data.startswith("getproduct_"):
            UserOperations.purchase_a_products(call, call.data.replace('getproduct_', ''))
        elif call.data.startswith("managecats_"):
            manage_categoriesbutton(call, call.data.replace('managecats_', ''))
    except Exception as e:
        logger.error(f"Callback Error: {e}")

# --- معالجة إضافة المنتجات (كما في كودك الأصلي) ---
@bot.message_handler(func=lambda message: message.text == "Manage Products 💼")
def manage_products_menu(message):
    if message.from_user.id == ADMIN_ID:
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        keyboard.add("Add New Product ➕", "List Product 🏷", "Delete Product 🗑️", "Home 🏘")
        bot.send_message(message.chat.id, "قائمة إدارة المنتجات:", reply_markup=keyboard)

# ====================================================
# --- تشغيل البوت بنظام الطنين المستمر (Polling) ---
# ====================================================
if __name__ == '__main__':
    print("🚀 Shop Bot is running in Polling mode...")
